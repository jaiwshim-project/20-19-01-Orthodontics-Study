...
| 자료유형 | 출처 | 제목 | 시간 | 포맷 | 참고이미지 |
|---------|------|------|------|------|---------|
| 슬라이드 | Kim's EZM | 슬라이드 40-45 (심화: 수직부정교합) | 6h | PPT | 수직부정교합 분석 다이어그램 |
| 교과서 | Kim Yong Eul | Chapter 9-10 (수직부정교합, 유지프로토콜) | 8h | PDF | Deep Bite vs Open Bite 비교 |
| 플랫폼 코드 | 20-19 Orthodontics | 알고리즘 분석 (GitHub) | 6h | GitHub | 플랫폼 아키텍처 다이어그램 |
| 케이스 분석 | Kim's Cases | 실제 환자 20케이스 분석 | 8h | Video/PDF | 임상 케이스 이미지 |
| 실습 프로젝트 | 플랫폼 개선 | 새 기능 개발 및 개선 | 2h | Code | 코드 구조 다이어그램 |
